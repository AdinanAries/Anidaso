...
function set_city_for_hotel_search(iata, city){
    ...
    localStorage.setItem("hotels_last_search_city", iata);
}

if(localStorage.getItem("hotels_last_search_city")){
    const city_name = aita_city_codes.filter(each => each.code === localStorage.getItem("hotels_last_search_city"));
    document.getElementById("hotels_where_search_input_fld").value = city_name[0].city;
}